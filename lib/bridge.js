...
const { PairingHost, PairingClient } = require('./pairing');
...
class Bridge extends EventEmitter {
    constructor() {
        ...
        // Session cipher from SPAKE2 pairing (replaces pre-shared secret for bridge traffic)
        this.sessionCipher = null;   // SessionCipher instance — AES-256-GCM with SPAKE2-derived key
        this._pairingHost = null;    // Active PairingHost instance (during `pair` command)
    ...
    disconnect() {
        ...
        // Destroy session cipher (zero key material)
        ...
        if (this._pairingHost) {
            this._pairingHost.cancel();
            this._pairingHost = null;
        }
    ...
    getStatus() {
        ...
        return {
            ...
            encryption: this.sessionCipher ? 'AES-256-GCM (SPAKE2 session key)' : 'AES-256 (pre-shared)',
            auth: this.sessionCipher ? 'PIN-PAIRED' : 'PRE-SHARED SECRET',
    ...
    // ═══════════════════════════════════════════════════════
    //  PIN-BASED PAIRING (SPAKE2)
    // ═══════════════════════════════════════════════════════

    /**
     * Set session cipher from a successful pairing.
     * Replaces the pre-shared SecureChannel for bridge traffic encryption.
     * @param {SessionCipher} cipher - SPAKE2-derived session cipher
     */
    setSessionCipher(cipher) {
        // Destroy any previous session cipher
        if (this.sessionCipher) this.sessionCipher.destroy();
        this.sessionCipher = cipher;

        // Swap the wsClient's encryption to use the session cipher
        if (this.wsClient) {
            this.wsClient.setSessionCipher(cipher);
        }

        logger.info('Session cipher installed — bridge traffic now uses SPAKE2-derived key');
    }

    /**
     * Start pairing as host: generates a PIN, listens for a client.
     * Returns the PairingHost instance so the REPL can display the PIN/port.
     * The caller should await pairingHost.start() for completion.
     */
    createPairingHost() {
        if (this._pairingHost) this._pairingHost.cancel();
        this._pairingHost = new PairingHost();
        return this._pairingHost;
    }

    /**
     * Connect to a pairing host as client with a PIN.
     * Returns promise resolving to { sessionKey, sessionCipher, peerHostname }.
     */
    async pairWithPin(host, port, pin) {
        const client = new PairingClient();
        return client.connect(host, port, pin);
    }
}
