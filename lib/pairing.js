...
const FW_RULE_NAME = 'OmniBridge-Pairing';

/**
 * Add a temporary Windows Firewall inbound rule for a TCP port.
 * Fails silently on non-Windows or if not running as admin.
 */
function addFirewallRule(port) {
    ...
    try {
        ...
        logger.info('Firewall rule added for pairing port', { port });
        return true;
    } catch (_) {
        logger.warn('Could not add firewall rule (not running as admin?)', { port });
        return false;
    }
}

/**
 * Remove the temporary OmniBridge pairing firewall rule.
 */
function removeFirewallRule() {
    ...
    try {
        ...
        // Rule may not exist or no admin rights — ignore
    }
}
...
class PairingHost {
    ...
    start() {
        return new Promise((resolve, reject) => {
            ...
            this.server.listen(this.port, '0.0.0.0', () => {
                ...
                // Open firewall for this port (Windows)
                this._fwAdded = addFirewallRule(this.port);
            });
        });
    }
    ...
    _cleanup() {
        ...
        if (this._fwAdded) { removeFirewallRule(); this._fwAdded = false; }
        this.pin = null;
    }
}
    ...
    connect(host, port, pin) {
        return new Promise((resolve, reject) => {
            ...
            const timeout = setTimeout(() => {
                done(new Error('Pairing timed out — host unreachable or firewall blocking port'));
                socket.destroy();
            }, 15000);
            ...
            socket.on('error', (err) => {
                clearTimeout(timeout);
                // Network-level errors are safe to report (no PIN info leaked)
                if (err.code === 'ECONNREFUSED') {
                    done(new Error('Connection refused — no pairing server on that port (check IP and port)'));
                } else if (err.code === 'ETIMEDOUT' || err.code === 'EHOSTUNREACH') {
                    done(new Error('Host unreachable — check IP, firewall, or network connectivity'));
                } else if (err.code === 'ECONNRESET') {
                    done(new Error('Connection reset by host — PIN may be wrong or expired'));
                } else {
                    done(new Error(`Connection error: ${err.code || err.message}`));
                }
            });
            ...
            socket.on('close', () => {
                clearTimeout(timeout);
                if (state !== 'DONE') {
                    done(new Error('Pairing failed — connection closed unexpectedly'));
                }
            });
        });
    }
