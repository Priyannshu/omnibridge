...
class Repl {
    ...
    cmdConnect(args) {
        ...
        if (pin) {
            ...
            this.bridge.pairWithPin(host, port, pin).then((result) => {
                ...
                this._interruptLog('[pair]  ✗ Pairing failed — wrong PIN, expired, or unreachable.', 'error');
            });
            return;
        }
    ...
    cmdPair() {
        ...
        // The PIN and port are available synchronously after start() begins
        setTimeout(() => {
            ...
            if (pin) {
                ...
                this.log('  │                                     │', 'sep');
                this.log(`  │           ${pin}                    │`, 'success');
                this.log('  │                                     │', 'sep');
                this.log(`  │  Port: ${String(port).padEnd(30)}│`, 'dim');
                this.log('  │                                     │', 'sep');
